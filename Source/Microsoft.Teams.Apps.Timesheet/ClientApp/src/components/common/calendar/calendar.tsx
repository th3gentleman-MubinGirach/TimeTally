...
interface ICalendarProps {
  ...
  onCalendarActiveDateChange?: (
    previousSelectedDate: Date,
    selectedDate: ITimesheet,
    isFrozen: boolean
  ) => void;
...
const Calendar: React.FunctionComponent<ICalendarProps> = (props) => {
  ...
  const [
    isDuplicateHoursActivated,
    setDuplicatedHoursActivated,
  ] = React.useState(false);
  const [calendarDates, setCalendarDates] = React.useState([] as ITimesheet[]);
  const [
    selectedWeekdaysToDuplicate,
    setSelectedWeekdaysToDuplicate,
  ] = React.useState([] as ITimesheet[]);
  const [
    filledEffortsOfRenderedWeek,
    setFilledEffortsOfRenderedWeek,
  ] = React.useState(0);
  ...
  function getCalendarDates() {
    ...
    let startDateOfWeek: Date = moment()
      .week(renderedWeek)
      .startOf('week')
      .startOf('day')
      .toDate();
    let endDateOfWeek: Date = moment()
      .week(renderedWeek)
      .endOf('week')
      .startOf('day')
      .toDate();
    ...
    for (
      let currentDate: Date = startDateOfWeek;
      ...
      let timesheet = props.timesheetData
        ? props.timesheetData.find(
            (timesheet: IUserTimesheet) =>
      ...
      if (timesheet) {
        calendarDates.push({
          date,
          hours: getTotalEfforts(timesheet),
          status: getTimesheetStatus(timesheet.projectDetails),
        });
        filledEffortsOfRenderedWeek +=
          calendarDates[calendarDates.length - 1].hours;
      } else {
  ...
  function getTimesheetStatus(projectDetails: IProjectDetails[]) {
    if (projectDetails) {
      for (let i = 0; i < projectDetails.length; i++) {
        let filledTimesheets = projectDetails[i].timesheetDetails.filter(
          (timesheet: ITimesheetDetails) =>
  ...
  function getDuplicateHoursSelectionClass(timesheet: ITimesheet) {
    ...
    if (canEffortsBeDuplicated(timesheet)) {
      let styleClass: string = props.isMobile
        ? 'cursor-pointer mobile select-to-duplicate'
        : 'cursor-pointer select-to-duplicate';

      let weekdayDetails = selectedWeekdaysToDuplicate.find(
        (weekdayDetails: ITimesheet) =>
          weekdayDetails.date.valueOf() === timesheet.date.valueOf()
      );
      styleClass =
        styleClass + (weekdayDetails ? ' selected-to-duplicate' : '');
  ...
  function isTimesheetDateFrozen(dateToCheck: Date, currentDate: Date) {
    ...
    if (
      moment(dateToCheck).startOf('day').toDate().valueOf() >
      moment().endOf('month').startOf('day').toDate().valueOf()
    ) {
      return true;
    }
    ...
    if (moment().date() >= props.timesheetFreezeDayOfMonth) {
      ...
      return (
        dateToCheck.valueOf() <
        moment(currentDate).startOf('month').startOf('day').valueOf()
      );
    }
  ...
  function onCalendarDateSelected(weekday: ITimesheet) {
    if (isDuplicateHoursActivated) {
      ...
      let weekdayDetails = selectedWeekdaysToDuplicate.find(
        (weekdayDetails: ITimesheet) =>
          weekdayDetails.date.valueOf() === weekday.date.valueOf()
      );
      let selectedWeekdays: ITimesheet[] = selectedWeekdaysToDuplicate
        ? cloneDeep(selectedWeekdaysToDuplicate)
        : [];
      ...
      if (!weekdayDetails) {
        ...
        selectedWeekdays = selectedWeekdays.filter(
          (day: ITimesheet) => day.date.valueOf() !== weekday.date.valueOf()
        );
      }

      props.onSelectedDatesChange &&
        props.onSelectedDatesChange(selectedWeekdays);
      ...
      props.onCalendarActiveDateChange &&
        props.onCalendarActiveDateChange(
          ...
          weekday,
          isTimesheetDateFrozen(weekday.date, moment().startOf('day').toDate())
        );
    }
  }
  ...
  function renderDays() {
    ...
    let weekdayElements: JSX.Element[] = weekdays.map(
      (weekday: ITimesheet, index: number) => {
        let day = weekday.date.getDay();
        let todaysDateClass =
          weekday.date.valueOf() === todaysDate.valueOf() ? 'todays-date' : '';
        let dateSelectionClass = getDuplicateHoursSelectionClass(weekday);

        if (weekday.date.valueOf() === selectedDate.valueOf()) {
          dateSelectionClass =
            dateSelectionClass +
            (props.isMobile ? ' mobile selected-date' : ' selected-date');
        }

        let colorCodeStyleForEfforts: string = getEffortsColorStyle(
          weekday.status
        );
        ...
        return (
          <Flex
            key={`weekday-${index}`}
            className={dateSelectionClass}
            column
            hAlign='center'
            space='between'
            data-testid={`calendar-date-${moment(weekday.date).format(
              'YYYY-MM-DD'
            )}`}
            onClick={() => onCalendarDateSelected(weekday)}
          >
            <Text
              className='week-day'
              content={
                props.isMobile
                  ? moment.weekdaysShort(day).charAt(0)
            ...
            <Text
              className={
                props.isMobile
                  ? `mobile date-text ${todaysDateClass}`
                  : `date-text ${todaysDateClass}`
              }
            ...
            {!props.isLoading ? (
              <Text
                className={
                  props.isMobile
                    ? `mobile hours-text ${colorCodeStyleForEfforts}`
                    : `hours-text ${colorCodeStyleForEfforts}`
                }
    ...
    return (
      <Flex className='calendar-days' space='around' fill>
        {weekdayElements}
      </Flex>
    );
  }
  ...
  function renderWeekAndMonthHeader() {
    ...
    return localize('calendarWeekAndMonth', {
      weekNumber: date.format('WW'),
      month,
      filledEffortsOfWeek: ('0' + filledEffortsOfRenderedWeek).slice(-2),
    });
  }
  ...
  function renderCalendarFooter() {
    return (
      <Flex className='calendar-footer' vAlign='center'>
        {isDuplicateHoursActivated ? (
          <Text
            data-testid='duplicate-efforts-placeholder'
            content={localize('duplicateEffortsPlaceholder')}
          />
        ) : null}
        <Flex.Item push>
          <div>
            {!props.isManagerView ? (
              <Button
                ...
                content={
                  isDuplicateHoursActivated
                    ? localize('duplicateEffortsSubmitButtonText')
                    : localize('duplicateEffortsButtonText')
                }
    ...
    <Flex data-tid={`calendar-component`} className='calendar' column>
      <Flex vAlign='center' space='between'>
        <Button
          ...
          onClick={onRequestPreviousCalendarWeek}
        />
        ...
        <Button
          ...
          onClick={onRequestNextCalendarWeek}
        />
      </Flex>
      {renderDays()}
      <Flex.Item push>{renderCalendarFooter()}</Flex.Item>
    </Flex>
  );
};
